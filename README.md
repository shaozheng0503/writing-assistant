...
还可以一键切换到**分段对比视图**：以原文的语义分段为对齐锚点，把三个技能各自的输出横向对齐到同一文章结构上，逐部分直观比较各技能的优劣。
...
- 思考面板带实时进度指示：思考中显示「已思考 Xs · Y 字」（每秒刷新），完成后显示「思考完成 · 总耗时 · 总字数」，对正文何时出现有明确预期
...
- 三列同时流式输出，逐字打出
- 推理模型（GLM-5/5.1/5.2, DeepSeek-V4-Pro 等）的思考过程用折叠面板展示，带秒级进度指示
- 完成后按空行拆成段落卡片，每段有编号、复制按钮
- 单击段落选中，双击段落收入拼接区，「加入拼接区」按钮批量收入
- 每列可单独重新生成（↻ 按钮），重新生成后分段对比自动重新对齐该列

### 分段对比视图

三个技能的分段方式各不相同，无法直接横向比较。分段对比通过「源文本对齐」解决：

1. **统一分段**：LLM 把原文按语义拆成 3~8 个标准段（标题 + 概要）——原文是三个技能共同改写对象，是天然对齐锚点
2. **映射**：每个技能生成完后，LLM 输出「段落编号 → 标准段编号」的数字数组，只做分组不改写文字，原始输出零细节丢失
3. **展示**：每个标准段一行，三个技能的对应输出横向并排；某技能无对应段显示占位；原文没有对应部分的「新增内容」（如结尾总结）集中展示在底部附加行

交互与自由视图完全一致：选中 / 复制 / 双击收入 / 整行收入拼接区，两套视图选中状态互通。

分段与对齐是结构化小任务，内部强制使用快速模型（deepseek-v4-flash，秒级返回），不受主模型选择影响——即使用 GLM-5 这类推理模型做正文改写，对比视图也能在正文完成后几秒~十几秒内就绪。
...
- 浅色主题（高对比度文字 + 6 级字号体系 + 三色视觉层次），长时间阅读友好
- 预填 API Key（.env 配置后用户打开即用）
- 五列布局：配图库 / human-writing / humanizer-zh / ljg-plain / 拼接区；顶栏可切换「自由视图 / 分段对比」
...
writing-workbench/
├── src/
│   ├── index.html      # 页面结构（输入页 + 结果页双视图 + lightbox）
│   ├── styles.css      # 样式（浅色主题 + 配图库 + 思考面板 + 对比视图 + lightbox）
│   ├── main.js         # 核心逻辑（LLM 调用、流式输出、分段对比、配图、拼接、导出）
...
- 分段/对齐/配图描述提取等辅助调用强制快速模型，LLM 解析失败时降级（原文空行分段 / 顺序分配），视图始终可用
